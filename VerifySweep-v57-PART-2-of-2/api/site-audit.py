...
def normalize_url(raw):
    ...
    if p.username or p.password: raise ValueError('Website URLs with embedded credentials cannot be audited.')
    try: port=p.port
    except ValueError: raise ValueError('Use a valid website port.')
    if port not in (None,80,443): raise ValueError('Only standard web ports 80 and 443 can be audited.')
    return raw
...
def assert_public_host(url):
    ...
    if p.username or p.password: raise ValueError('URLs with embedded credentials are not allowed.')
    try: port=p.port
