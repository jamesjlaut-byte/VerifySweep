...
def search_db(zipcode,q=''):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(f'''SELECT id,company,professional_name,credential,issuer,credential_source,city,state,postal_code,website,phone
                            FROM pro_directory WHERE {' AND '.join(where)} ORDER BY company,professional_name LIMIT 50''',params)
            rows=cur.fetchall()
            keys=['id','company','holder','credential','issuer','source','city','state','zip','website','phone']
    ...
    def do_GET(self):
        try:
            ...
            self.sendj(200,{'results':results,'count':len(results),'verified_only':True,'database_connected':bool(DB),
              'official_sources':{'csia':'https://web.csia.org/CSIA-Certified','nfi':'https://www.nficertified.org/public/find-an-nfi-pro/'},
